...
|指令形式|说明|示例|架构|
|:--|:--|:--|:--|
|vadd_<数据类型简写>|64位数据相加。||v7, A32, A64|
|vaddq_<数据类型简写>|128位数据相加。||v7, A32, A64|
|vaddl_<数据类型简写>|长指令相加，结果数据是操作数位数的两倍。||v7, A32, A64|
|vaddl_high_<数据类型简写>|操作数a的后半部分（高位）元素加上操作数b的后半部分（高位）元素，结果数据位数扩大两倍存储。|
