...
> 64位数据相加；支持架构：v7, A32, A64。

<br>

**2. vaddq_<数据类型简写>**

> 128位数据相加；支持架构：v7, A32, A64。

<br>

**3. vaddl_<数据类型简写>**

> 长指令，结果数据是操作数位数的两倍；支持架构：v7, A32, A64。

<br>

**4. vaddl_high_<数据类型简写>**

> 长指令，操作数a和操作数b的后半元素（高位数据）相加，结果位数扩大两倍存储；支持架构：A64。
...
> 宽指令，操作数a和b通道数相同，操作数a的位数是操作数b的两倍，结果位数与操作数a相同。支持架构：v7, A32, A64。
...
> 操作数a的元素加上操作数b的后半部分（高位）对应位置元素；架构支持：A64。
...
> 操作数a的元素加上对应通道操作数b的元素，并将所加结果右移一位存储，向下取整；架构支持：v7, A32, A64。
...
**8. vrhadd_<数据类型简写> （64位数据）/  vrhaddq_<数据类型简写> （128位数据）**

> 操作数a的元素加上对应通道操作数b的元素，并将所加结果右移一位存储，向上取整；架构支持：v7, A32, A64。
...
**9.  vqadd_<数据类型简写> （64位数据）/ vqaddq_<数据类型简写> （128位数据）**

> 饱和指令；操作数a和操作数b对应通道元素相加，超出范围的自动限制在该数据类型表示范围内；架构支持：v7, A32, A64。
...
**10. vuqadd_<数据类型简写> （64位数据）/ vuqaddq_<数据类型简写> （128位数据）**
